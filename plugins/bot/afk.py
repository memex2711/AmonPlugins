...
async def add_afk(user_id, details):
    await dB.set_var(user_id, str(user_id), details, AFK_CATEGORY)

async def is_afk(user_id):
    data = await dB.get_var(user_id, str(user_id), AFK_CATEGORY)
...
async def remove_afk(user_id):
    await dB.remove_var(user_id, str(user_id), AFK_CATEGORY)
...
async def active_afk(_, message: Message):
    ...
    if verifier:
        ...
        try:
            ...
            seenago = Tools.get_readable_time((int(time.time() - timeafk)))
...
async def chat_watcher_func(_, message):
    ...
    if message.entities:
        possible = ["/afk", f"/afk@{BOT_USERNAME}", "/brb", f"/brb@{BOT_USERNAME}"]
...
__MODULE__ = "Afk"
__HELP__ = """
<blockquote expandable>📋 <b>Afk Commands</b>
